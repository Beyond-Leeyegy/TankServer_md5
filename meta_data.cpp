...
int PutGameRunFrame(byte* packet,Game& game , int frame_num){
	...
	//md5
	log("debug info::start md5\r\n");
	
	PutMD5(packet,GAME_RUN_FRAME,frame_num,end);

	log("debug info::end md5;start PutMap end pos:%d\r\n",end);
	//map
	PutMap(packet,game,end);
...
/*
	8*8 = 64 bit
	using frame_num and tag 
*/
void PutMD5(byte*packet,char tag ,int frame_num,int&end){
       //set type
	 char type[2] ;
        type[0] = tag;
        type[1] = '\0';
	...
	MD5 md5;
        ...
        md5.update(str);
        char p[33];
        string tmp = md5.toString();
        tmp.copy(p, 8, 0);
        *(p + 8) = '\0';
	
	log("Putting md5:%s\r\n",p);      
 	log("end : %d",end);
	WtriteStringToBuffer(packet, p, end, strlen(p), end);
